...
ArmPlugin::ArmPlugin()
  ...
  for (uint32_t n = 0; n < DOF; n++) resetPos[n] = 0.0f;
...
void ArmPlugin::onCollisionMsg(ConstContactsPtr& contacts) {
  ...
  for (unsigned int i = 0; i < contacts->contact_size(); ++i) {
    if (strcmp(contacts->contact(i).collision2().c_str(), COLLISION_FILTER) ==
        0)
      continue;
    ...
    /*
    / TODO - Check if there is collision between the arm and object, then issue
    learning reward
    /
    */
...
bool ArmPlugin::updateAgent() {
  ...
  if (CUDA_FAILED(cudaPackedToPlanarBGR((uchar3*)inputBuffer[1], inputRawWidth,
    ...
    printf(
        "ArmPlugin - failed to convert %zux%zu image to %ux%u planar BGR "
        "image\n",
  ...
  /*
  / TODO - Increase or decrease the joint velocity based on whether the action
  is even or odd
  /
  */

  float velocity =
      0.0;  // TODO - Set joint velocity based on whether action is even or odd.
  ...
  /*
  / TODO - Increase or decrease the joint position based on whether the action
  is even or odd
  /
  */
  float joint =
  ...
  if (joint < JOINT_MIN) joint = JOINT_MIN;

  if (joint > JOINT_MAX) joint = JOINT_MAX;
...
bool ArmPlugin::updateJoints() {
  if (testAnimation)  // test sequence
  {
    ...
    if (animationStep > ANIMATION_STEPS) {
      ...
      if (!loopAnimation) testAnimation = false;
    } else if (animationStep == ANIMATION_STEPS / 2) {
      ResetPropDynamics();
    }
  ...
  if (newReward && agent != NULL) {
    if (DEBUG) {
      printf("ArmPlugin - issuing reward %f, EOE=%s  %s\n", rewardHistory,
             ...
             (rewardHistory > 0.1f) ? "POS+" : (rewardHistory > 0.0f)
                                                   ? "POS"
                                                   : (rewardHistory < 0.0f)
                                                         ? "    NEG"
                                                         : "       ZERO");
    ...
    if (endEpisode) {
      ...
      if (rewardHistory >= REWARD_WIN) successfulGrabs++;
      ...
      for (uint32_t n = 0; n < DOF; n++) vel[n] = 0.0f;
    }
  }
}
}
